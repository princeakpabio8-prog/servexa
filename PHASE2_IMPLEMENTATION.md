...
- Creates example-provider-call-id records with outcome, sentiment, action_required
...
Two new tables:
- **example-provider-call-id**: Standard templates seeded (Loan Recovery, Payment Reminder, etc.)
- **example-provider-call-id**: Stores human-provided instructions, amount, currency, due date per call
...
- Creates example-provider-call-id record
...
4. Check Supabase:
   ...
   SELECT * FROM example-provider-call-id WHERE call_id = 'test-call-id';
...
3. **Step 1**: Select a customer (for example, "Example Customer")
...
4. Check database:
   ```sql
   SELECT COUNT(*) FROM example-provider-call-id WHERE call_id = 'xxx';
   ```
...
**Check**:
1. Is `example-provider-call-id` record created? (query Supabase)
