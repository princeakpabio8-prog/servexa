...
Templates are represented in the current call-instruction screen and seeded in the `example-provider-call-id` table. The selected template is passed to the backend as `template_name`.
...
3. The function optionally creates a `example-provider-call-id` row.
...
7. The webhook updates `calls`, inserts `example-provider-call-id`, creates `activities`, and creates `follow_ups` for escalations.
